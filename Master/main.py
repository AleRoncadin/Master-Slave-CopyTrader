...
CRED_FILE = "mt5_credentials.json"
...
def load_mt5_credentials(file_path: str = CRED_FILE):
    try:
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            creds = json.load(f)
        if "login" in creds:
            try: creds["login"] = int(creds["login"])
...
def prompt_and_validate_mt5_credentials(max_attempts: int = 3, file_path: str = CRED_FILE):
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            ok = mt5.initialize(path=path, login=login, password=password, server=server, portable=True)
            if ok:
                mt5.shutdown()
...
def order_to_dict(o):
    return {
        ...
        "time": safe_timestamp(getattr(o, "time", None)),
...
def deal_to_dict(d):
    return {
        ...
        "time": safe_timestamp(getattr(d, "time", None)),
...
class StorageSupabase:
    ...
    def store(self, event_type: str, payload: dict):
        ...
        ticket = symbol = typ = volume = sl = tp = None

        if isinstance(payload, dict):
            if "position" in payload and isinstance(payload["position"], dict):
                p = payload["position"]
                ticket = p.get("ticket"); symbol = p.get("symbol"); typ = map_type_to_text(p.get("type"))
                volume = p.get("volume"); sl = p.get("sl"); tp = p.get("tp")
            elif "order" in payload and isinstance(payload["order"], dict):
                o = payload["order"]
                ticket = o.get("ticket"); symbol = o.get("symbol"); typ = map_type_to_text(o.get("type"))
                volume = o.get("volume"); sl = o.get("sl"); tp = o.get("tp")
            elif "deal" in payload and isinstance(payload["deal"], dict):
                d = payload["deal"]
                ticket = d.get("ticket"); symbol = d.get("symbol"); typ = map_type_to_text(d.get("type"))
                volume = d.get("volume")
        ...
        p = payload.get("position") if isinstance(payload, dict) else None
        record["price_open"] = float(p.get("price_open")) if isinstance(p, dict) and p.get("price_open") is not None else None
...
class MT5Monitor:
    ...
    def snapshot_deals_recent(self, lookback_seconds=10):
        now = datetime.now(timezone.utc)
        from_time = now.timestamp() - lookback_seconds
        deals = mt5.history_deals_get(from_time, now.timestamp())
    ...
    def detect_and_store(self):
        ...
        for t, old_pos in list(self.last_positions.items()):
            if t not in current_positions:
                payload = build_base_context()
                payload.update({"event_type": "position_closed", "position": old_pos})
                self.storage.store("position_closed", payload)
        for t, pos in current_positions.items():
            if t in self.last_positions:
                old = self.last_positions[t]
                diffs = {}
                for k in ("sl", "tp"):
                    if old.get(k) != pos.get(k):
                        diffs[k] = {"old": old.get(k), "new": pos.get(k)}
                if diffs:
                    payload = build_base_context()
                    payload.update({"event_type": "position_modified", "ticket": t, "position": pos, "diffs": diffs})
                    self.storage.store("position_modified", payload)

        current_orders = self.snapshot_orders()
        for t, od in current_orders.items():
            if t not in self.last_orders:
                payload = build_base_context()
                payload.update({"event_type": "order_placed", "order": od})
                self.storage.store("order_placed", payload)
        for t, old_od in list(self.last_orders.items()):
            if t not in current_orders:
                payload = build_base_context()
                payload.update({"event_type": "order_removed_or_executed", "order": old_od})
                self.storage.store("order_removed_or_executed", payload)
        for t, od in current_orders.items():
            if t in self.last_orders:
                old = self.last_orders[t]
                diffs = {}
                for k in ("sl", "tp"):
                    if old.get(k) != od.get(k):
                        diffs[k] = {"old": old.get(k), "new": od.get(k)}
                if diffs:
                    payload = build_base_context()
                    payload.update({"event_type": "order_modified", "ticket": t, "order": od, "diffs": diffs})
                    self.storage.store("order_modified", payload)

        recent_deals = self.snapshot_deals_recent(lookback_seconds=int(self.poll_interval * 2 + 2))
        for d in recent_deals:
            key = d["ticket"]
            if key not in self.seen_deals:
                payload = build_base_context()
                payload.update({"event_type": "deal", "deal": d})
                self.storage.store("deal", payload)
                self.seen_deals.add(key)
    ...
    def run(self):
        ...
        try:
            ...
            self.last_orders = self.snapshot_orders()
            for d in self.snapshot_deals_recent(lookback_seconds=60):
